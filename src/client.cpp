...
static int32_t print_response(const uint8_t *data, size_t size) {
    ...
    switch (static_cast<RespTag>(data[0])) {
    case RespTag::Nil:
        printf("(nil)\n");
        return 1;
    case RespTag::Err:
        if (size < 1 + 8) { msg("bad response"); return -1; }
        {
            ...
            memcpy(&code, &data[1], 4); memcpy(&len, &data[1 + 4], 4);
    ...
    case RespTag::Str:
        ...
        {
            uint32_t len = 0; memcpy(&len, &data[1], 4);
    ...
    case RespTag::Int:
        if (size < 1 + 8) { msg("bad response"); return -1; }
        {
            int64_t val = 0; memcpy(&val, &data[1], 8);
            printf("(int) %ld\n", val);
            return 1 + 8;
        }
    case RespTag::Dbl:
        if (size < 1 + 8) { msg("bad response"); return -1; }
        {
            double val = 0; memcpy(&val, &data[1], 8);
            printf("(dbl) %g\n", val);
            return 1 + 8;
        }
    case RespTag::Arr:
...
static int32_t read_res(int fd) {
    ...
    if (err) { msg(errno == 0 ? "EOF" : "read() error"); return err; }
    uint32_t len = 0; memcpy(&len, rbuf, 4);
    ...
    if (rv > 0 && static_cast<uint32_t>(rv) != len) { msg("bad response"); rv = -1; }
    return rv;
}
...
int main(int argc, char **argv) {
    ...
    if (connect(fd, reinterpret_cast<const struct sockaddr *>(&addr), sizeof(addr))) die("connect");
